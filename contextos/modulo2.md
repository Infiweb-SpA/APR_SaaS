Perfecto. Aquí tienes el contexto actualizado del **Módulo 2** con los cambios aplicados:
...
### 2.2 Constantes de configuración RBAC (Actualizadas)
...
ALL_MODULES = ['auth', 'portal_socio', 'partners', 'readings', 'billing']
...
elif any(has_permission(m, 1) for m in ['partners', 'readings', 'billing']): → main.admin_dashboard
...
- **Incluye CSRF token** `{{ csrf_token() }}` en formulario.
...
5. **Fila expandible permisos** (`id="perms-{{user.id}}"` hidden) – grid 3 cols con selects `perm_<module>` (0/1/2) pre-seleccionados según `user.get_permission_level(module)`. **Los módulos disponibles son:** `auth`, `portal_socio`, `partners`, `readings`, `billing`.

**JS inline:** `togglePerms(userId)` alterna clase `.hidden` en fila de permisos.

---

## 8. Integración con `app/__init__.py`
...
| **Módulos RBAC Activos** | Lista fija `ALL_MODULES` (5 items): `auth`, `portal_socio`, `partners`, `readings`, `billing`. |
| **JS RUT** | Cargar `rut_val.js` en cualquier template con inputs `.rut-input` (vía `extra_scripts` o layout). |
| **Protección CSRF** | Implementado con `Flask-WTF` + `CSRFProtect` (`{{ csrf_token() }}` en todos los formularios). |
...
2. **Módulo 3 (Main/Portal)** – `_redirect_by_role()` envía a `main.admin_dashboard` o `main.socio_portal`.
3. **Módulo 4 (Partners)** – Requiere `@permission_required('partners', 1/2)` en sus rutas.
4. **Módulo 5 (Readings)** – Requiere `@permission_required('readings', 2)` para captura.
5. **Módulo 6 (Billing)** – Requiere `@permission_required('billing', 2)` para cierre de mes.
